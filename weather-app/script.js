...
const favChips = document.getElementById('fav-chips');
...
const weatherCodes = {
    ...
    56: { desc: 'Freezing drizzle',     icon: 'fa-cloud-rain',          sky: 'rain' },
    57: { desc: 'Freezing drizzle',     icon: 'fa-cloud-rain',          sky: 'rain' },
    66: { desc: 'Freezing rain',        icon: 'fa-cloud-showers-heavy', sky: 'rain' },
    67: { desc: 'Freezing rain',        icon: 'fa-cloud-showers-heavy', sky: 'rain' },
    ...
    77: { desc: 'Snow grains',          icon: 'fa-snowflake',           sky: 'snow' },
    80: { desc: 'Rain showers',         icon: 'fa-cloud-showers-heavy', sky: 'rain' },
    81: { desc: 'Rain showers',         icon: 'fa-cloud-showers-heavy', sky: 'rain' },
    82: { desc: 'Violent showers',      icon: 'fa-cloud-showers-heavy', sky: 'rain' },
    85: { desc: 'Snow showers',         icon: 'fa-snowflake',           sky: 'snow' },
    86: { desc: 'Snow showers',         icon: 'fa-snowflake',           sky: 'snow' },
    95: { desc: 'Thunderstorm',         icon: 'fa-bolt',                sky: 'storm' },
    96: { desc: 'Thunderstorm + hail',  icon: 'fa-bolt',                sky: 'storm' },
    99: { desc: 'Thunderstorm + hail',  icon: 'fa-bolt',                sky: 'storm' },
};
...
async function getCoordinates(place) {
    let res, data;
    try {
        res = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(place)}&count=1&language=en&format=json`);
        data = await res.json();
    } catch {
...
// Best-effort reverse geocoding for the "use my location" button.
// Falls back to a generic label if the lookup fails — never blocks
// the actual weather load on this.
...
async function loadWeather(lat, lon, placeName) {
    ...
    try {
        ...
        // Prayer times and air quality load independently — neither
        // should block the main dashboard if their APIs are slow or down.
...
// ==================================================================
// Prayer times (Aladhan API — Egyptian General Authority method)
// ==================================================================
...
const PRAYER_ORDER = [
    ...
    { key: 'Maghrib', label: 'Maghrib', icon: 'fa-mountain-sun' },
...
async function loadPrayerTimes(lat, lon) {
    ...
    try {
        ...
        const res = await fetch(`https://api.aladhan.com/v1/timings/${dateStr}?latitude=${lat}&longitude=${lon}&method=5`);
        if (!res.ok) throw new Error('bad prayer response');
        const data = await res.json();
        if (data.code !== 200) throw new Error('bad prayer payload');
        panel.style.display = '';
        renderPrayerTimes(data.data);
    } catch (err) {
        // Non-critical feature — hide the panel rather than showing broken data.
        panel.style.display = 'none';
        console.error('Prayer times unavailable:', err);
    }
}
...
function renderPrayerTimes(data) {
    ...
    PRAYER_ORDER.forEach((p) => {
        const timeStr = prayerTimings[p.key].split(' ')[0];
        const card = document.createElement('div');
        ...
        card.innerHTML = `
            ...
            <h4>${p.label}</h4>
            <div class="ptime">${formatPrayerClock(timeStr)}</div>
        `;
        grid.appendChild(card);
    });
    ...
    prayerCountdownInterval = setInterval(updateNextPrayer, 30000);
}

function formatPrayerClock(hhmm) {
...
function updateNextPrayer() {
    ...
    for (const p of PRAYER_ORDER) {
        const [h, m] = prayerTimings[p.key].split(' ')[0].split(':').map(Number);
        const t = new Date();
        t.setHours(h, m, 0, 0);
        if (t > now) { next = p; nextTime = t; break; }
    }
    if (!next) {
        // All five prayers have passed — count down to tomorrow's Fajr.
        next = PRAYER_ORDER[0];
        const [h, m] = prayerTimings.Fajr.split(' ')[0].split(':').map(Number);
        nextTime = new Date();
    ...
    const diffMin = Math.max(0, Math.floor((nextTime - now) / 60000));
    ...
    document.querySelectorAll('.prayer-time').forEach((el) => {
        el.classList.toggle('is-next', el.dataset.key === next.key);
    });
}
...
async function loadAirQuality(lat, lon) {
    ...
    try {
        ...
        if (!res.ok) throw new Error('bad air quality response');
        const data = await res.json();
        el.textContent = aqiLabel(data.current ? data.current.us_aqi : null);
    } catch (err) {
        el.textContent = '—';
        console.error('Air quality unavailable:', err);
    }
}
...
// ==================================================================
// Favorite cities (localStorage, for quick one-tap switching)
// ==================================================================

function getFavorites() {
    try { return JSON.parse(localStorage.getItem('weather-favorites')) || []; }
    catch { return []; }
}
...
function findFavoriteIndex(list, lat, lon) {
    return list.findIndex((f) => Math.abs(f.lat - lat) < 0.01 && Math.abs(f.lon - lon) < 0.01);
}

function updateFavButton(lat, lon) {
    const isFav = findFavoriteIndex(getFavorites(), lat, lon) !== -1;
...
function renderFavChips() {
    ...
    favChips.innerHTML = '';
    favChips.style.display = favs.length ? 'flex' : 'none';

    favs.forEach((f) => {
        const chip = document.createElement('button');
        chip.className = 'fav-chip';
        chip.type = 'button';
        chip.innerHTML = `<span>${f.name}</span><span class="remove"><i class="fas fa-xmark"></i></span>`;
        chip.addEventListener('click', (e) => {
            if (e.target.closest('.remove')) {
                e.stopPropagation();
                const updated = getFavorites().filter((x) => !(x.lat === f.lat && x.lon === f.lon));
                saveFavorites(updated);
                renderFavChips();
                if (lastLat != null) updateFavButton(lastLat, lastLon);
                return;
            }
            loadWeather(f.lat, f.lon, f.name);
        });
...
favBtn.addEventListener('click', () => {
    ...
    const favs = getFavorites();
    const idx = findFavoriteIndex(favs, lastLat, lastLon);
    if (idx === -1) {
        favs.push({ lat: lastLat, lon: lastLon, name: lastPlaceName });
    } else {
        favs.splice(idx, 1);
    }
    saveFavorites(favs);
    updateFavButton(lastLat, lastLon);
    renderFavChips();
});
...
searchBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        alertContainer.style.display = 'block';
        alertText.textContent = err.message === 'network'
